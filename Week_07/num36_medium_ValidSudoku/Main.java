package question_bank.num36_medium_ValidSudoku;
...
class Solution {
    ...
    public boolean isValidSudoku(char[][] board) {
      for(int i = 0; i < 9; i ++){
        ...
        for(int j = 0; j < 9; j ++){
          // 由于传入为char，需要转换为int，减48  (char类型的数字减去48即为int类型，'0'的ASCII码即为48)
          /*
          * board[i][j]表示了对固定的一行ii，遍历列jj(即每次检查一整行)
            board[j][i]board[j][i]表示了对固定的一列ii，遍历行jj(即每次检查一整列)
            board[3∗(i/3)+j/3][3∗(i%3)+j%3]表示了对固定的宫ii，遍历格jj(即每次检查一个宫)
          * */
          int h = board[i][j] - 48;
          int v = board[j][i] - 48;
          int s = board[3 * (i / 3) + j / 3][3 * (i % 3) + j % 3] - 48;
          // "."的ASCII码为46，故小于0代表着当前符号位"."，不用讨论  (大于0判断的作用：表示的是如果)
          if(h > 0){
